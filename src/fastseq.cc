...
FastSeq::FastSeq() : quant_(false) {}

void FastSeq::addInputVector(Vector& vec, int32_t ind) const {
  if (quant_) {
...
std::shared_ptr<const DictionarySeq> FastSeq::getDictionarySeq() const {
  return dict_;
}

const Args FastSeq::getArgs() const {
  return *args_.get();
}

std::shared_ptr<const Matrix> FastSeq::getInputMatrix() const {
  return input_;
}

std::shared_ptr<const Matrix> FastSeq::getOutputMatrix() const {
  return output_;
}

int32_t FastSeq::getWordId(const std::string& word) const {
  return dict_->getId(word);
}

int32_t FastSeq::getSubwordId(const std::string& word) const {
...
void FastSeq::getSubwordVector(Vector& vec, const std::string& subword)
    const {
  vec.zero();
...
bool FastSeq::checkModel(std::istream& in) {
  int32_t magic;
...
void FastSeq::signModel(std::ostream& out) {
...
void FastSeq::saveModel(const std::string path) {
...
void FastSeq::loadModel(const std::string& filename) {
...
void FastSeq::loadModel(std::istream& in) {
  ...
  dict_ = std::make_shared<DictionarySeq>(args_, in);
...
void FastSeq::printInfo(real progress, real loss, std::ostream& log_stream) {
...
std::vector<int32_t> FastSeq::selectEmbeddings(int32_t cutoff) const {
  Vector norms(input_->size(0));
  input_->l2NormRow(norms);
  std::vector<int32_t> idx(input_->size(0), 0);
  std::iota(idx.begin(), idx.end(), 0);
  auto eosid = dict_->getId(DictionarySeq::EOS);
...
void FastSeq::quantize(const Args qargs) {
  if (args_->model != model_name::sup) {
    throw std::invalid_argument(
...
void FastSeq::cbow(Model& model, real lr,
                    const std::vector<int32_t>& line) {
...
void FastSeq::skipgram(Model& model, real lr,
                        const std::vector<int32_t>& line) {
  std::uniform_int_distribution<> uniform(1, args_->ws);
  for (int32_t w = 0; w < line.size(); w++) {
    int32_t boundary = uniform(model.rng);
...
std::tuple<int64_t, double, double> FastSeq::test(
    std::istream& in,
    int32_t k,
    real threshold) {
...
void FastSeq::getSentenceVector(
    std::istream& in,
...
void FastSeq::ngramVectors(std::string word) {
...
void FastSeq::precomputeWordVectors(Matrix& wordVectors) {
  Vector vec(args_->dim);
  wordVectors.zero();
  for (int32_t i = 0; i < dict_->nwords(); i++) {
    std::string word = dict_->getWord(i);
...
void FastSeq::analogies(int32_t k) {
  std::string word;
...
void FastSeq::trainThread(int32_t threadId) {
...
void FastSeq::loadVectors(std::string filename) {
...
void FastSeq::train(const Args args) {
  args_ = std::make_shared<Args>(args);
  dict_ = std::make_shared<DictionarySeq>(args_);
  if (args_->input == "-") {
    // manage expectations
...
int FastSeq::getDimension() const {
    return args_->dim;
}

bool FastSeq::isQuant() const {
  return quant_;
}
